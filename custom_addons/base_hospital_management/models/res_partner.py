...
class ResPartner(models.Model):
    ...
    def create_patient(self, post):
        """Create/reuse a patient and create the outpatient visit.

        The legacy dashboard only created the OPD row when an existing patient
        card was selected. The OWL dashboard uses this method for both existing
        and walk-in patients, so the outpatient creation is intentionally shared.
        """
        if not post:
            return False
        patient = False
        if post.get('patient_id'):
            patient = self.sudo().browse(int(post['patient_id'])).exists()
        if not patient:
            patient = self.sudo().search([('name', '=', post.get('op_name'))], limit=1)
        if not patient:
            patient_vals = {
                'name': post.get('op_name'),
                'phone': post.get('op_phone'),
                'blood_group': post.get('op_blood_group'),
                'rh_type': post.get('op_rh'),
                'gender': post.get('op_gender'),
            }
            if post.get('op_dob'):
                patient_vals['date_of_birth'] = post['op_dob']
            patient = self.sudo().create(patient_vals)

        outpatient_vals = {
            'patient_id': patient.id,
            'op_date': post.get('date') or fields.Date.context_today(self),
            'reason': post.get('reason'),
            'slot': post.get('slot') or 0.0,
        }
        if post.get('doctor'):
            outpatient_vals['doctor_id'] = self.env['doctor.allocation'].sudo().browse(int(post['doctor'])).id
        out_patient = self.env['hospital.outpatient'].sudo().create(outpatient_vals)
        return {'patient_id': patient.id, 'outpatient_id': out_patient.id, 'op_reference': out_patient.op_reference}
